using System;
using System.Collections.Generic;
using System.Text;
using System.Data;
using Sistem.DB.Model;
using Sistem.DB.Repo;

namespace Sistem.DB.Service
{
    public class AdmUserMenuService : ServiceBase
    {
        private readonly AdmUserMenuRepo _itemRepo;

        public AdmUserMenuService(string ConStr)
        {
            ProgramConnection.ConStr = ConStr;
            _itemRepo = new AdmUserMenuRepo();
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing == true)
            {
                _itemRepo.Dispose();
                // Managed kaynaklar icin 
            }
            base.Dispose(disposing);
            // UnManaged kaynaklar icin
        }

        public int Ekle(AdmUserMenu item)
        {
            return _itemRepo.Insert(item);
        }

        public int Guncelle(AdmUserMenu item)
        {
            return _itemRepo.Update(item);
        }

        public int Sil(int id)
        {
            return _itemRepo.Delete(id);
        }

        public List<AdmUserMenu> GetItemClass(int userRef, int menuRef)
        {
            return _itemRepo.GetItemClass(userRef, menuRef);
        }

        public DataSet GetAll(int userRef, int menuRef)
        {
            return _itemRepo.GetAll(userRef, menuRef);
        }

        public void VerileriSil(int kul)
        {
            _itemRepo.VerileriSil(kul);
        }

    }
}
